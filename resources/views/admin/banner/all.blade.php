    ...
    <div class="col-xl-12">
        <div class="card">
            <div class="card-header bg-secondary">
              <div class="row">
                <div class="col-md-8 card_header_title text-white"><i class="fa fa-gg-circle"></i>All banner Information</div>
                <div class="col-md-4 text-end card_header_btn">
                  <a class="btn btn-sm btn-dark" href="{{url('dashboard/banner/add')}}"><i class="fa fa-plus-circle"></i>Add banner</a>
                </div>
              </div>
            </div>
            <div class="card-body">
              <table class="table table-bordered table-striped custom_table">
                ...
                @foreach ($all as $data)
                  <tr>
                    <td>{{Str::words($data->ban_title,5)}}</td>
                    <td>{{str::words($data->ban_subtitle,5)}}</td>
                    <td>{{$data->ban_button}}</td>
                    <td>{{$data->ban_url}}</td>
                    <td>                   
                       @if($data->ban_image!='')
                         <img height="70" src="{{asset('uploads/'.$data->ban_image)}}"/>
                       @else
                      ...
                      <div class="btn-group" role="group">
                        <div class="btn-group" role="group">
                          ...
                          <ul class="dropdown-menu">
                            <li><a class="dropdown-item" href="{{url('dashboard/banner/view/'.$data->ban_slug)}}">View</a></li>
                            <li><a class="dropdown-item" href="{{url('dashboard/banner/edit/'.$data->ban_slug)}}">Edit</a></li>
                            <li><a class="dropdown-item" href="#" id="softdelete" data-bs-toggle="modal" data-bs-target="#softdelete-modal" data-id="{{$data->ban_id}}">Delete</a></li>
