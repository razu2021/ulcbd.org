            ...
            <div class="card-header bg-secondary">
              <div class="row">
                ...
                <div class="col-md-4 text-end card_header_btn">
                  <a class="btn btn-sm btn-dark" href="{{url('dashboard/website/news/add')}}"><i class="fa fa-plus-circle"></i>Add news</a>
                </div>
              </div>
            </div>
            <div class="card-body">
              ...
              @foreach ($all as $data)
                  <tr>
                    <td>{{Str::words($data->news_heading,5)}}</td>
                    <td>{{Str::words($data->news_caption,5)}}</td>
                    <td>{{Str::words($data->news_title,5)}}</td>
                    <td>{{Str::words($data->news_info,5)}}</td>
                    <td>{{Str::words($data->news_date,5)}}</td>
                    <td>
                    @if($data->news_image!='')
                      <img height="70" src="{{asset('uploads/'.$data->news_image)}}"/>
                    @else
                      ...
                      <div class="btn-group" role="group">
                        <div class="btn-group" role="group">
                          ...
                          <ul class="dropdown-menu">
                            <li><a class="dropdown-item" href="{{url('dashboard/website/news/view/'.$data->news_slug)}}">View</a></li>
                            <li><a class="dropdown-item" href="{{url('dashboard/website/news/edit/'.$data->news_slug)}}">Edit</a></li>
                            <li><a class="dropdown-item" href="#" id="softdelete" data-bs-toggle="modal" data-bs-target="#softdelete-modal" data-id="{{$data->news_id}}">Delete</a></li>
...
<div id="softdelete-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="standard-modalLabel" aria-hidden="true">
  <div class="modal-dialog">
      <form method="post" action="{{url('dashboard/website/news/softdelete')}}">
        @csrf
